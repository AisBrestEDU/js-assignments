...
function getPolynom() {
    ...
    return x => [...arguments].reduce((acc, val, i) => acc + val * x**(n - i), 0);
}
...
function logger(func, logFunc) {
    return (...args) => {
        let strArgs = JSON.stringify(args).slice(1, -1);
...
function partialUsingArguments(fn) {
    let partialArgs = [...arguments].slice(1);
    
    return function() {
        let args = partialArgs.concat([...arguments]);
        return fn(...args);
    };
}
