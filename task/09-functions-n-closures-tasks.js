...
function getPolynom() {
    let args = arguments;
    return  args.length === 0? null : function(x){
        let polynom = 0
        for (let i = 0; i < args.length; i++)
            polynom += args[i] * x**(args.length-i-1);
        return polynom;
    }
}
...
function logger(func, logFunc) {
    return function(x){
        let str = func.name+'('+JSON.stringify(Array.from(arguments)).slice(1,-1)+')';
        logFunc(str+' starts')
...
function partialUsingArguments(fn) {
    ...
    return function(){return fn.apply(null, args.concat(Array.from(arguments)))}
}
