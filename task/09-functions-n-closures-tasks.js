 ...
 */
function getPolynom() {
    const arg = Array.from(arguments).reverse();
    return (x) => {
        const checkIsNaN = (x) => {
            return isNaN(x) ? 0 : x;
        }
        ...
        let [cCoef, bCoef, aCoef] = arg;

        aCoef = checkIsNaN(aCoef); 
        bCoef = checkIsNaN(bCoef);
        cCoef = checkIsNaN(cCoef);

        return aCoef * Math.pow(x, 2) + bCoef * x + cCoef;
    }
}
...
function logger(func, logFunc) {
    return function() {
        ...
        const logString = `${func.name}(${args})`;
...
function partialUsingArguments(fn) {
    const args = Array.from(arguments).slice(1, Array.from(arguments).length);
    return function() {
