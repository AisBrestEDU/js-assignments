 ...
 */
function getPolynom() {
    return x => [...arguments].reduce((previous, current, index) =>
        previous + current * Math.pow(x, arguments.length - 1 - index), 0);
}
...
function logger(func, logFunc) {
    return function (...args) {
        let logStart = func.name + '(' + JSON.stringify(args).slice(1, -1) + ') starts';
        logFunc(logStart);
        let result = func(...args);
        let logEnd = func.name + '(' + JSON.stringify(args).slice(1, -1) + ') ends';
        logFunc(logEnd);
        return result;
    }
}
