...
function isLeapYear(date) {
    let testedYear = new Date(date).getFullYear();
    if (testedYear % 4 != 0) {
        return false;
    }
    else if (testedYear % 100 != 0) {
        return true;
    }
    else if (testedYear % 400 != 0) {
        return false;
    }
    else {
        return true;
...
function angleBetweenClockHands(date) {
    let grad = Math.abs(0.5 * (60 * (date.getUTCHours() % 12 || 12) - 11 * date.getUTCMinutes()));
    return (grad > 180 ? 360 - grad : grad) * Math.PI / 180;
}
