...
function getAverage(value1, value2) {
    return ((value1 + value2)/2 == Infinity)?Number.MAX_VALUE:(value1 + value2)/2;
}
...
function getAngleBetweenVectors(x1, y1, x2, y2) {
    var skalarProd = x1*x2 + y1*y2;
    var vecModuls1 = Math.sqrt(Math.pow(x1,2) + Math.pow(y1,2));
    var vecModuls2 = Math.sqrt(Math.pow(x2,2) + Math.pow(y2,2));      
    return Math.acos(skalarProd/(vecModuls1 * vecModuls2));
}
...
function getLastDigit(value) {
    return Number.parseInt(value.toString().slice(-1));
}
