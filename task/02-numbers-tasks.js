...
function getAngleBetweenVectors(x1, y1, x2, y2) {
    ...
    return (angl<0)?(Math.PI+angl):(Math.PI-angl);
}
...
function roundToPowerOfTen(num, pow) {
    ...
    if((num % Math.pow(10,pow))<Math.pow(10,pow)/2){
        return num-(num % Math.pow(10,pow));
    }else{
        return num-(num % Math.pow(10,pow))+Math.pow(10,pow);
    }
}
...
function isPrime(n) {
    ...
    for(var i=2; i<=Math.sqrt(n); i++) if(!(n % i)) return false; 
    return true;
}
