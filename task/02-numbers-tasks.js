...
function getAngleBetweenVectors(x1, y1, x2, y2) {
    const vectorsLength = x1 * x2 + y1 * y2;
    const firstVectorScalarProduct = Math.sqrt(x1 * x1 + y1 * y1);
    const secondVectorScalarProduct = Math.sqrt(x2 * x2 + y2 * y2);
    return Math.acos(vectorsLength / (firstVectorScalarProduct * secondVectorScalarProduct));
}
