...
function getAverage(value1, value2) {
    ...
    let avarage = (value1 + value2) * 0.5;
    if (avarage < Number.MAX_VALUE) {
        return avarage;
...
function getAngleBetweenVectors(x1, y1, x2, y2) {
    return Math.acos(((x1 * x2) + (y1 * y2)) / Math.hypot(x1 * x1 , y1 * y1) * Math.hypot(x2 * x2 , y2 * y2));
}
...
function roundToPowerOfTen(num, pow) {
    if (pow >= 0) return Math.round(num / Math.pow(10, pow.toFixed())) * Math.pow(10, pow);
}
...
function toNumber(value, def) {
    if (typeof Number(value) === "number" && !isNaN(Number(value)) && Number(value) !== 0) return value;
    else return def;
}
