...
function getAngleBetweenVectors(x1, y1, x2, y2) {
    function coordinate(a, b, c, d) {
        return (a*c + b*d);
    };
    function length(a, b) {
        return Math.sqrt(Math.pow(a, 2) + Math.pow(b, 2));
    };
    let cos = (coordinate(x1, y1, x2, y2)) / (length(x1, y1) * length(x2, y2));
...
function getLastDigit(value) {
    return [value].join('')[[value].join('').length - 1];
}
...
function isPrime(number) {
    let firstNum = 2;
    let secondNum = Math.sqrt(number);
    while (firstNum <= secondNum) {
        if (number % firstNum++ < 1) return false;
    };
    return true;
}
