...
function getAngleBetweenVectors(x1, y1, x2, y2) {
    let scalarMultiply = x1 * x2 + y1 * y2;
    let v1Length = Math.hypot( x1, y1 );
    let v2Length = Math.hypot( x2, y2 );
    return Math.acos( scalarMultiply / ( v1Length * v2Length ) );
}
...
function toNumber(value, def) {
    return  Number.parseInt(value) || Number.parseFloat(value) ?
            value : def;
}
