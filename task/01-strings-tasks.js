...
function extractNameFromTemplate(value) {
    const valuesToDelete = [/Hello/gi, /!/gi, /,/gi];
    let name = value;
    for (let item of valuesToDelete) {
...
function removeFirstOccurrences(str, value) {
    ...
    result.splice(index, value.length);
    return (result.join(""));
}
...
function encodeToRot13(str) {
    ...
    for (let i = 0; i < str.length; i++) {
 	    result += output[input.indexOf(str[i])] || str[i];
    }
    return result;
}
...
function getCardId(value) {
    ...
    const cardSuit = card.splice(-1)[0];
    const indexX = x.indexOf(card.join(""));
    const indexY = y.indexOf(cardSuit);
    const id = indexX + (indexY*x.length);
   return id;
}
