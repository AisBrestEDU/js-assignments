...
function getRectangleString(width, height) {
  ...
  let body = '\n' + '│' + ' '.repeat(width - 2) + '│';
  let lastLine = '\n' + '└' + '─'.repeat(width - 2) + '┘' + '\n';
  return firstLine + body.repeat(height - 2) + lastLine;
}
...
function encodeToRot13(str) {
  ...
  for (let i = 0; i < str.length; i++) {
    ...
    if (code > 64 && code < 91) {
      let cipheredCode = str.charCodeAt(i) + 13;
      if (cipheredCode > 90) {
        res += String.fromCharCode(65 + 13 - (91 - str.charCodeAt(i)));
      } else {
        res += String.fromCharCode(cipheredCode);
      }
    } else if (code > 96 && code < 123) {
      let cipheredCode = str.charCodeAt(i) + 13;
      if (cipheredCode > 122) {
        res += String.fromCharCode(97 + 13 - (123 - str.charCodeAt(i)));
      } else {
        res += String.fromCharCode(cipheredCode);
      }
    } else {
...
function isString(value) {
  if (value === undefined || value === null) {
    ...
    return value.__proto__ === String.prototype;
  }
}
