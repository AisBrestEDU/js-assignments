...
/**
 * Extacts a name from template string 'Hello, First_Name Last_Name!'.
 *r
 * @param {string} value
 * @return {string}
 *
...
function removeFirstOccurrences(str, value) {
    return str.replace(value, '').split(' ').join(' ');
}
...
function getRectangleString(width, height) {
    let top='', bot='', middle='', horizontal='─', space=' ', vertical='│'; 
    
    if(width < 2 || height < 2) {
        return false;   
    }

    top='┌'+horizontal.repeat(width-2)+'┐'+'\n'; 
    middle=vertical+space.repeat(width-2)+vertical+'\n'; 
    bot='└'+horizontal.repeat(width-2)+'┘'+'\n'; 

    return `${top}${middle.repeat(height-2)}${bot}`; 
}
