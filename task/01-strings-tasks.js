...
function getRectangleString(width, height) {
    const topString = `┌${'─'.repeat(width - 2)}┐\n`;
    const centerString = ' '.repeat(width - 2);
    const centerStrings = `│${centerString}│\n`.repeat(height - 2);
    const bottomString = `└${'─'.repeat(width - 2)}┘\n`;
    return `${topString}${centerStrings}${bottomString}`;
}
...
function encodeToRot13(str) {
    ...
    const searchRegExp = /[A-Z]/gi;
    return str.replace(searchRegExp, char => output[input.indexOf(char)]);
}
...
function getCardId(value) {
    ...
    return initialDesk.indexOf(value);
}
