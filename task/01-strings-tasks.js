...
function extractNameFromTemplate(value) {
    ...
    return `${words[1]} ${words[2].replace(words[2].charAt((words[2].length)-1), '')}`;
}
...
function getRectangleString(width, height) {
  const horizPart = '─'.repeat(width-2);
  const vertPart = "│"+ ' '.repeat(width-2)+"│\n";
  let result = "┌" + horizPart + "┐\n" + vertPart.repeat(height-2) + "└" + horizPart + "┘\n";
return result;
}
...
function encodeToRot13(str){
    const normalCodes = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz";
    const substitutions = 'NOPQRSTUVWXYZABCDEFGHIJKLMnopqrstuvwxyzabcdefghijklm';
    let encoded = "";
    for(let ch of str) {
        let index = normalCodes.indexOf(ch);
        encoded += index === -1 ? ch : substitutions[index];
    }
