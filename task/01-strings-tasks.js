...
function getRectangleString(width, height) {
        ...
        for (let i = 0; i < height; i++) {
            ...
            if (i != 0 && i < height -1) {
                tempStr = ' ';
                tempStr = '│' + tempStr.repeat(width -2) + '│\n';
            } else {
                tempStr = '─';
                if (i == 0) {
                    tempStr = '┌' + tempStr.repeat(width -2) + '┐\n';
...
function encodeToRot13(str) {
    ...
    for (let i = 0; i < input.length; i++) {
        myMap.set(input[i], output[i]);
    }

    for (let strElelemt of str.split('')) {
        result += myMap.get(strElelemt);
    }
...
function getCardId(value) {
    ...
    for (let num in cardArray) {
       myMap.set(cardArray[num], num);
    }
