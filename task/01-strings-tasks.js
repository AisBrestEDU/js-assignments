...
function extractNameFromTemplate(value) {
    return value.substr(7).split("!").join("");
}
...
function getRectangleString(width, height) {
        ...
        for(let i = 1; i <= height; i++) {
            switch (i) {
                case 1:
                    rectangle += "┌" + horizontLine.repeat(width-2) + "┐" + transferLine;
                    break;
                case height:
                    rectangle += "└" + horizontLine.repeat(width-2) + "┘" + transferLine;
                    break; 
                default: 
                    rectangle += verticalLine + insideLine.repeat(width-2) + verticalLine + transferLine;
            }        
        }
        return rectangle;
}
...
function encodeToRot13(str) {
          ...
          return "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz".charAt(
                 "NOPQRSTUVWXYZABCDEFGHIJKLMnopqrstuvwxyzabcdefghijklm".indexOf(a)
          );
        } );   
}
