...
function extractNameFromTemplate(value) {
  return isString(value) ? value.replace('Hello, ', '').replace('!', '') : '';
}
...
function getFirstChar(value) {
  return isString(value) ? value.charAt(0) : '';
}
...
function repeatString(value, count) {
  return isString(value) ? value.repeat(count) : '';
}
...
function removeFirstOccurrences(str, value) {
  return isString(str) && isString(value) ? str.replace(value, '') : '';
}
...
function unbracketTag(str) {
  return isString(str) ? str.slice(1, -1) : '';
}
...
function encodeToRot13(str) {
  ...
  for (let i = 0; i < str.length; i++) {
    if (str[i].charCodeAt() >= 65 && str[i].charCodeAt() <= 90 || str[i].charCodeAt() >= 97 && str[i].charCodeAt() <= 122) {
      if (str[i].charCodeAt() < 78 || str[i].charCodeAt() >= 97 && str[i].charCodeAt() < 110) {
        strEncode += String.fromCharCode(str[i].charCodeAt() + 13);
      } else {
        strEncode += String.fromCharCode(str[i].charCodeAt() - 13);
      }
    } else {
      strEncode += String.fromCharCode(str[i].charCodeAt());
    }
  }
  return strEncode;
}
...
function getCardId(value) {
  ...
  const cardSuits = ['\u2663', '\u2666', '\u2665', '\u2660'];
  return cardRanks.indexOf(value.slice(0, -1)) + cardSuits.indexOf(value.slice(-1)) * cardRanks.length;
}
