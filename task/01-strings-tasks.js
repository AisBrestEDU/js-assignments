...
function getRectangleString(width, height) {
    ...
    let printRectangle = printBorderLine('┌', '┐', '─', width);

    for (let i = 0; i < height - 2; i++) 
    {
        printRectangle += printBorderLine('│', '│', ' ', width); 
    }
    ...
    function printBorderLine(startBorder, finishBorder, space, width) {
        let borderLine = startBorder;
        borderLine += repeatString(space, width-2)
        borderLine += finishBorder + '\n';
...
function encodeToRot13(str) {
    ...
    for (var i = 0; i < str.length; i++) 
    {
        ...
        {
            encodeString += String.fromCharCode(str.charCodeAt(i) - 13);
        } 
        else if (77 < (str.charCodeAt(i)) && (str.charCodeAt(i)) <= 90) 
        {
            encodeString += String.fromCharCode(str.charCodeAt(i) - 13);
        }
         else if ((str.charCodeAt(i)) < 64) {
            encodeString += String.fromCharCode(str.charCodeAt(i));
        }
         else
        {
            encodeString += String.fromCharCode(str.charCodeAt(i) + 13);
...
function getCardId(value) {
    ...
    let cardsSuits = '♣♦♥♠';
    let cardsNumbers = 'A234567891JQK';
    let tempCardSuits = 13 * cardsSuits.indexOf(value[value.length - 1]);

    return cardsNumbers.indexOf(value[0]) + tempCardSuits;
}
