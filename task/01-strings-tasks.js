...
function getRectangleString(width, height) {
    ...
    rectangle = rectangle.concat("─".repeat(width - 2));
    rectangle = rectangle.concat("┐\n");
...
function encodeToRot13(str) {
    ...
    for (let i = 0; i < str.length; i += 1) {
        let index = lowerCaseLetters.indexOf(str[i]);

        if (index != -1) {
            result = result.concat(lowerCaseLetters[(index + 13) % 26]);
        }
        else {
            index = upperCaseLetters.indexOf(str[i]);

            if (index != -1)
                result = result.concat(upperCaseLetters[(index + 13) % 26]);
            else 
