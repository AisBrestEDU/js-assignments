...
function getRectangleString(width, height) {
    ...
    let bottom = '└ ┘\n';
    if (width > 2) {
        top = top.replace(/ /, '─'.repeat(width - 2));
        bottom = bottom.replace(/ /, '─'.repeat(width - 2));
    } else {
        top = top.replace(/ /, '');
...
function encodeToRot13(str) {
    ...
    for (let i = 0; i < str.length; i++) {
        k = str.charCodeAt(i);
        if (k < charCodeOfA || k > charCodeOfz) {
            k = k;
        } else if (k <= charCodeOfZ - ROT) {
            k = k + ROT;
        } else if (k + ROT >= charCodeOfZ && k <= charCodeOfZ) {
            k = charCodeOfA - charCodeOfZ + k + ROT - 1;
        } else if (k >= charCodeOfa && k + ROT <= charCodeOfz) {
            k = k + ROT;
        } else if (k + ROT > charCodeOfz && k <= charCodeOfz) {
            k = charCodeOfa + k - charCodeOfz -1 + ROT;
        }
