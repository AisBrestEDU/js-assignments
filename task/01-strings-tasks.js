    ...
    for(let h = 1; h <= height; h++) {
        switch (h) {
            case 1: string = string + `┌${'─'.repeat(width - 2)}┐\n` 
                break
            case height: string = string + `└${'─'.repeat(width - 2)}┘\n`
                break
            default: string = string + `│${' '.repeat(width - 2)}│\n`
        }        
    }
    return string
}
...
function encodeToRot13(str) {
    ...
    const encodedrAlph = 'NOPQRSTUVWXYZABCDEFGHIJKLMnopqrstuvwxyzabcdefghijklm'
    const encodedStr = []

    for (let char of str) {
        if (char.match(/[^\dA-Z]/gi)) {
            encodedStr.push(char)
        } else {
            const encodedChar = encodedrAlph[alph.indexOf(char)]
            encodedStr.push(encodedChar)
        }
    }
