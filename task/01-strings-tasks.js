...
function getRectangleString(width, height) {
  function line(width, ch_beg, ch_mid, ch_end) {
    let l = '' + ch_beg;
    for (let j = 1; j < width - 1; j++) l += ch_mid;
    l += ch_end;
    return l + '\n';
  }
  let rect = '';
  rect += line(width, '┌', '─', '┐');
  for (let i = 1; i < height - 1; i++) rect += line(width, '│', ' ', '│');
...
function encodeToRot13(str) {
  let firstHalf = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz';
  let lastHalf = 'NOPQRSTUVWXYZABCDEFGHIJKLMnopqrstuvwxyzabcdefghijklm';
  let index = (x) => firstHalf.indexOf(x);
  let translate = (x) => (index(x) > -1 ? lastHalf[index(x)] : x);
