...
function isInsideCircle(circle, point) {
    const distanceToCenter = Math.sqrt(Math.pow(point.x - circle.center.x, 2) + Math.pow(point.y - circle.center.y, 2));
...
function getIntervalString(a, b, isStartIncluded, isEndIncluded) {
    let arr = [a, b];
    arr.sort((a, b) => a - b);
    let StartIncluded = isStartIncluded ? '[' : '(';
    let EndIncluded = isEndIncluded ? ']' : ')';
...
function timespanToHumanString(startDate, endDate) {
    ...
    let month = ms / 2592000000.0000005;
...
function getMatrixProduct(m1, m2) {
    ...
    for (var k = 0; k < m2[0].length; k++) {
        for (var i = 0; i < m1.length; i++) {
            var temp = 0;
            for (var j = 0; j < m2.length; j++) temp += m1[i][j] * m2[j][k];
