...
function doRectanglesOverlap(rect1, rect2) {
    ...
    let x11 = rect1.top;
    let y11 = rect1.left;
    let x14 = x11 + rect1.height;
    let y14 = y11 + rect1.width;

    let x21 = rect2.top;
    let y21 = rect2.left;
    let x24 = x21 + rect2.height;
    let y24 = y21 + rect2.width;

    return !(x11 > x24 || y11 > y24 || x14 < x21 || y14 < y21)
...
function isBracketsBalanced(str) {
    ...
    let cl = [']','}',')','>'];
    for(let i = 0; i < str.length; i++){
        if(op.indexOf(str[i])>=0){
            tmp.push(op.indexOf(str[i]));
        }
        else{   
            if(cl[tmp.pop()] != str[i]){
                return false;
 ...
 *
 * @example:
 *   ['/web/images/image1.png', '/web/images/image2.png']  => '/web/images/' "/web/images/"
 *   ['/web/assets/style.css', '/web/scripts/app.js',  'home/setting.conf'] => '' "/"
 *   ['/web/assets/style.css', '/.bin/mocha',  '/read.me'] => '/' "//"
 *   ['/web/favicon.ico', '/web-scripts/dump', '/webalizer/logs'] => '/' "//wweebb"
 */
function getCommonDirectoryPath(pathes) {
    // throw new Error('Not implemented');
    let path0 = pathes[0];
    let a = path0.slice(0, path0.lastIndexOf('/')+1);
    let s = '';
    let b = true;
...
function getMatrixProduct(m1, m2) {
    // throw new Error('Not implemented');
    let rowA = m1.length;
    let colA = m1[0].length;
    let rowsB = m2.length;
    let colB = m2[0].length;
    let matrix = [];
    
    for(let t = 0; t < rowA; t++){
        matrix[t] = [];
    }
    for(let n = 0; n < rowA; n++){ //rows
        for(let i = 0; i < colB; i++){ //cols
            let scalar = 0;
            for(let j = 0; j < colA; j++){
                scalar +=  m1[n][j]*m2[j][i];
                matrix[n][i] = scalar;
            }
        }
    }
